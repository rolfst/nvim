    ...
    dap.configurations[ext] = {
        ...
        },
        {
            ...
            websocketAddress = function()
                return string.match(
                    vim.api.nvim_exec(
                        '!docker logs [container-name] |& grep -oE "ws.*" | tail -1',
                        true
                    ),
                    "ws://.*"
                )
...
local mason_codelldb = global.mason_path .. "/packages/codelldb/codelldb"
local codelldb_path = vim.fn.executable(mason_codelldb) == 1 and mason_codelldb or (os.getenv("CODELLDB_PATH") or mason_codelldb)
